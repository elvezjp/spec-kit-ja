# ご注意ください。
このリポジトリは、Github Spec kitのテンプレートなどについて、試験的に日本語化するためのものです。2025年9月9日にクローンし、一部日本語化しています。オリジナルのSpec kitの記載がほぼそのまま残っております。試験用のリポジトリですので、Spec kitをご利用したい方は、オリジナルのリポジトリをご参照ください。
